function [EEG, ev_val] = removeEpoch(EEG, ev_val, rejected_epochs)
    ...
    %  rejected_epochs   =  array of epochs to reject e.g. [1 2 5 78] or
    %                       [3]
    %
    % Returns:
    %  EEG               =  eeglab EEG-dataset (epoched) if all removed->
    %                       return empty matrix []
    %  ev_val            =  updated array of event-validity data
    %                       [channel epoch]


    % check if the rejected_epochs array is smaller than the epoch count
    % for the event (otherwise this would mess listboxes etc. in the gui)
