function saveMatFile(file, setid, EEG, ALLEEG, event_validity, ...
    ...
    % Saves EEG-study as a .mat-file for full Eegtool-support
    % (EEGDATA-struct)
    %
    % Parameters:
    ...
    %  event_validity   = cell-matrix describing validity of events after
    %                     artefact detection and manual correction
    %  urevent_validity = original event_validity matrix
    %  removed_epochs   = containers.Map on event removal information
    %                     (keys:event ids)
    %  vinfo            = video-info-struct extracted from the eegmplayer-
    %                     object (optional, if no video use only 7
    %                     parameters)
