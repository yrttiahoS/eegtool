function  visualize2d(fnames, condition, datamatrix, xdata, ydata, ...
    ...
    %  datamatrix = cell table with i cells. Cell i is the datamatrix for
    %               fnames{i}
    ...
    %  channels   = cell table with vector content of channels to plot for
    %               the i:th datamatrix 
    ...
    h.subjtext = uicontrol('Style', 'text', 'string', 'File', 'units', ...
                           'normalized', 'position', ...
                           [0.01 0.955 0.03 0.03], ...
    ...
    h.group_condition_button = uicontrol('Style', 'pushbutton', ...
    ...
    h.group_view_button = uicontrol('Style', 'togglebutton', 'string', ...
                                    'Group view', 'Enable', 'off', ...
    ...
    h.cranget = uicontrol('Style', 'text', 'string', 'Color limits', ...
                          'horizontalalignment', 'center', 'units', ...
                          'normalized', 'position', [0.01 0.05 0.06 0.03]);

    h.cmaxe = uicontrol('Style', 'edit', 'string', '10', ...
                        'horizontalalignment', 'center', 'units', ...
                        'normalized', 'position', [0.16 0.05 0.05 0.03]);

    h.cmine = uicontrol('Style', 'edit', 'string', '-10', ...
                        'horizontalalignment', 'center', 'units', ...
                        'normalized', 'position', [0.10 0.05 0.05 0.03]);

    h.eventtext = uicontrol('Style', 'text', 'string', ...
                            'Choose sample type', 'units', ...
                            'normalized', 'position', ...
                            [0.01 0.01 0.10 0.03]);

    sampletypes = {'mean', 'max', 'min', 'max_lat', 'min_lat'};

    h.samplepopup = uicontrol('Style', 'popupmenu', 'string', ...
                              sampletypes, 'units', 'normalized', ...
                              'position', [0.12 0.01 0.09 0.03]);
    ...
    h.ymaxe = uicontrol('Style', 'edit', 'string', '40', ...
                        'horizontalalignment', 'center', 'units', ...
                        'normalized', 'position', [0.39 0.05 0.05 0.03]);

    h.ymine = uicontrol('Style', 'edit', 'string', '10', ...
                        'horizontalalignment', 'center', 'units', ...
                        'normalized', 'position', [0.32 0.05 0.05 0.03]);               

    h.xmine = uicontrol('Style', 'edit', 'string', 'min', 'units', ...
                        'normalized', 'position', [0.32 0.01 0.05 0.03]);

    h.xmaxe = uicontrol('Style', 'edit', 'string', 'max', 'units', ...
                        'normalized', 'position', [0.39 0.01 0.05 0.03]);

    h.save_results_button = uicontrol('Style', 'pushbutton', 'string', ...
    ...
    h.haxes = drawTopoAxis(h.visualize2d, chanlocs, axdim, 'Time(ms)', ...
                           'Frequency(Hz)');

    set(h.group_view_button, 'callback', {@group_view_callback, h, ...
                                          fnames, datamatrix, xdata, ...
                                          ydata, channels, chanlocs});
    set(h.group_fname_button, 'callback', {@group_fname_callback, h, ...
                                           fnames, datamatrix, xdata, ...
                                           ydata, channels, chanlocs});
    set(h.group_condition_button, 'callback', ...
        {@group_condition_callback, h, condition, datamatrix, xdata, ...
         ydata, channels, chanlocs});
    set(h.subjpopup, 'callback', {@edit_callback, h, datamatrix, xdata, ...
                                  ydata, channels, chanlocs});
    set(h.cmine, 'callback', {@edit_callback, h, datamatrix, xdata, ...
                              ydata, channels, chanlocs});
    set(h.cmaxe, 'callback', {@edit_callback, h, datamatrix, xdata, ...
                              ydata, channels, chanlocs});
    set(h.save_results_button, 'callback', {@save_results_callback, h, ...
                                            fnames, datamatrix, xdata, ...
                                            ydata, channels, ...
                                            eventcount, chanlocs});
...
function edit_callback(~, ~, h, datamatrix, xdata, ydata, channels, ...
                       chanlocs)
    % callback for re-referencing-button and rereferencing-edit
    ...
    if istoggled
        ...
        drawTf(h, cdatamatrix{subj}, cxdata{subj}, cydata{subj}, ...
               cchannels{subj}, chanlocs);
    else
        drawTf(h, datamatrix{subj}, xdata{subj}, ydata{subj}, ...
               channels{subj}, chanlocs);
    end

function group_view_callback(hObject, ~, h, fnames, datamatrix, xdata, ...
                             ydata, channels, chanlocs)

    istoggled = get(hObject, 'value');

    % TODO -> format userdata, transfer combined to userdata etc.
    ...
    if istoggled
        ...
        set(h.subjpopup, 'callback', {@edit_callback, h, cdatamatrix, ...
                                      cxdata, cydata, cchannels, ...
                                      cchanlocs});
        drawTf(h, cdatamatrix{1}, cxdata{1}, cydata{1}, cchannels{1}, ...
               chanlocs);
    else
        % normal view
        set(h.subjpopup, 'value', 1);
        ...
        set(h.subjpopup, 'callback', {@edit_callback, h, datamatrix, ...
                                      xdata, ydata, channels, chanlocs});
        drawTf(h, datamatrix{1}, xdata{1}, ydata{1}, channels{1}, ...
               chanlocs);
    end

function group_condition_callback(~,~, h, condition, datamatrix, xdata, ...
                                  ydata, channels, chanlocs)
    ...
    for i = selected_conditions
        ...
        participants_with_this_condition = find(strcmp(condition, ...
                                                       uniqcondition{i}));
    ...
    if get(h.group_view_button, 'value')
        ...
        set(h.subjpopup, 'callback', {@edit_callback, h, cdatamatrix, ...
            cxdata, cydata, cchannels, cchanlocs});
        set(h.subjpopup, 'string', sets);
    end

function group_fname_callback(~, ~, h, fnames, datamatrix, xdata, ...
                              ydata, channels, chanlocs)
    ...
    combined = getappdata(gcf, 'combined');
    len = length(combined);
    setname = inputdlg('Give name to a set', 'Prompt', 1, ...
                       {['Set ' num2str(len+1)]});
...
function save_results_callback(~, ~, h, fnames, datamatrix, xdata, ...
                               ydata, channels, eventcount, chanlocs)
    % callback for save results-button

    save_results(h, fnames, datamatrix, xdata, ydata, channels, ...
                 eventcount, chanlocs);

function save_results(h, fnames, datamatrix, xdata, ydata, channels, ...
                      eventcount, chanlocs)
    ...
    ind_ymin = find(ydata{1}>= ...
                    str2num(get(h.ymine, 'String'))-0.1, 1, 'first');
    ind_ymax = find(ydata{1}<= ...
                    str2num(get(h.ymaxe, 'String'))+0.1, 1, 'last');
    ...
    for k = subjvector
        ...
        writeSampleRow(fid, SAMPLE, fnames{k}, stype, 'T-F', ...
                       num2str(eventcount{k}), num2str(xmin), ...
                       num2str(xmax), num2str(ymin), num2str(ymax));
    end
 ...
 function drawTf(h, data, xdata, ydata, channels, chanlocs)
    ...
    for i = channels
        ...
        text(xscale(2), yscale(2)-0.1*(yscale(2)-yscale(1)), 30, ...
             chanlabels, 'fontunits', 'normalized', 'fontsize', 0.25, ...
        ...
        set(h.haxes(i), 'clim', ...
            [str2num(get(h.cmine, 'string')) str2num(get(h.cmaxe, 'string'))]);
