function [events, winopts] = inputEventsGui(EEG, settings)
    % GUI that collects the parameters to perform epoching. Blocks
    % execution.
    %
    % Parameters:
    ...
    % define ui elements
    h.descl = uicontrol('Style', 'listbox', 'string', type, 'Max', ...
                        length(type), 'Min', 0, 'position', ...
                        [10 130 130 160], 'horizontalalignment', 'center');

    h.geneb = uicontrol('Style', 'pushbutton', 'string', ...
                        'Generate epochs', 'horizontalalignment', ...
                        'center', 'position', [10 10 130 30]);

    h.emine = uicontrol('Style', 'edit', 'string', settings{1}, ...
                        'position', [90 90 50 30]);

    h.emaxe = uicontrol('Style', 'edit', 'string', settings{2}, ...
                        'position', [90 50 50 30]);

    h.emint = uicontrol('Style', 'text', 'string', 'Epoch start', ...
                        'position', [10 90 70 30]);

    h.emaxt = uicontrol('Style', 'text', 'string', 'Epoch end', ...
