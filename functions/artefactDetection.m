function [ev_val, disp_val] = artefactDetection(EEG, type, treshold)
    % Performs artefact detection to datavector. Returns rating for the
    % input vector.
    %
    % Parameters:
    % data          =  1-dimensional vector containing EEG-data points
    % treshold      =  A comparison value to the analysis: 
    %                  function(data) smaller than this -> marked good.                        
    %
    % Output:
    ...
    for i=1:EEG.nbchan
        ...
        for j=1:EEG.trials %length(EEG.epoch)
            ...
            switch type
                ...
                case 'Max difference'
                    % if difference between min and max of data exceeds
                    % treshold
