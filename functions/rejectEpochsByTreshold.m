...
% Reject epochs from epoched EEG-dataset and 2-dimensional ev_val-matrix.
% Ev_val indicates which epochs are good and which are bad.
% 
% Parameters:
%  EEG        =  eeglab EEG-dataset (epoched)
%  ev_val     =  matrix of values [channel epoch] containing values 0-9: 0 good,
%                1 bad, 9 interpolated
%  treshold   =  what is the maximum count of bad channels in one epoch. 
%                If exceeded -> epoch removed. 
% Returns:
%  new_EEG    =  Same EEG-struct as parameter but without epochs that
%                contained more than treshold of bad epochs
%  new_ev_val =  New ev_val containing updated information after epoch removal
...
% form the treshold condition (percentage, obsolete)
% treshold = round(ALLEEG(j).nbchan*treshold_persentage/100);

for k=1:EEG.trials
    % roll throught all the events for detecting whether there are "bad"
    % epochs
