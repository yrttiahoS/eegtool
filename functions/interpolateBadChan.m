function [interpolated_EEG, interpolated_ev_val] = interpolateBadChan(EEG, ev_val)
% Function takes input an epoched EEG-dataset and 2-dimensional ev_val-matrix.
% Ev_val indicates which epochs are good and which are bad and therefore
...
for k=1:EEG.trials
    ...
    % [temp_ALLEEG, event_indices] = pop_selectevent(ALLEEG, 'event', i);
    % decided to do this with my own conversion because it proved to be
    % easier
    %%%%%%%%%%%%%%%%%%%%%%%%
    temp_EEG = EEG;
...
interpolated_EEG = EEG;
interpolated_ev_val = ev_val;