...
%  axisid     = {xid, yid} cell table with 2 strings. Xid labels x-axis and Yid labels y-axis.
...
h.difference_button = uicontrol('Style', 'pushbutton', 'string', 'Difference', ...
...
h.ymaxtext = uicontrol('Style', 'text', 'string', 'Ymax', ...
                   'horizontalalignment', 'left', 'units', 'normalized', 'position', [0.01 0.09 0.04 0.03]);

h.ymaxedit = uicontrol('Style', 'edit', 'string', limits(2), ...
                   'horizontalalignment', 'center', 'units', 'normalized', 'position', [0.06 0.09 0.04 0.03]);

h.ymintext = uicontrol('Style', 'text', 'string', 'Ymin', ...
                   'horizontalalignment', 'left', 'units', 'normalized', 'position', [0.01 0.05 0.04 0.03]);
               
h.yminedit = uicontrol('Style', 'edit', 'string', limits(1), ...
                   'horizontalalignment', 'center', 'units', 'normalized', 'position', [0.06 0.05 0.04 0.03]);

h.samplet = uicontrol('Style', 'text', 'string', 'Sample type', 'HorizontalAlignment', 'left', ...
...
h.samplep = uicontrol('Style', 'popupmenu', 'string', sampletypes, ...
                     'units', 'normalized', 'position', [0.09 0.01 0.09 0.03]);

h.ranget = uicontrol('Style', 'text', 'string', 'Range(x-axis)', 'HorizontalAlignment', 'left',...
                   'units', 'normalized', 'position', [0.19 0.01 0.08 0.03]);
                 
h.xmine = uicontrol('Style', 'edit', 'string', 'min', ...
                   'units', 'normalized', 'position', [0.28 0.01 0.05 0.03]);

h.xmaxe = uicontrol('Style', 'edit', 'string', 'max', ...
                   'units', 'normalized', 'position', [0.34 0.01 0.05 0.03]);

h.ranget2 = uicontrol('Style', 'text', 'string', 'Range2(x-axis)', 'visible', 'off', 'HorizontalAlignment', 'left',...
                   'units', 'normalized', 'position', [0.19 0.05 0.08 0.03]);
               
h.xmine2 = uicontrol('Style', 'edit', 'string', 'min', 'visible', 'off',...
                   'units', 'normalized', 'position', [0.28 0.05 0.05 0.03]);

h.xmaxe2 = uicontrol('Style', 'edit', 'string', 'max', 'visible', 'off',...
                   'units', 'normalized', 'position', [0.34 0.05 0.05 0.03]);
               
h.range2_helpt = uicontrol('Style', 'text', 'string', 'SAMPLE = SAMPLE(range)-SAMPLE(range2)', ...
                           'visible', 'off', 'HorizontalAlignment', 'left', ...
                           'units', 'normalized', 'position', [0.40 0.05 0.25 0.03]);

h.topoplot_button = uicontrol('Style', 'pushbutton', 'string', 'Topoplot', ...
                     'units', 'normalized', 'position', [0.70 0.01 0.13 0.03]);

h.extract_raw_button = uicontrol('Style', 'pushbutton', 'string', 'Extract raw', ...
                     'units', 'normalized', 'position', [0.85 0.05 0.13 0.03]);

h.extract_button = uicontrol('Style', 'pushbutton', 'string', 'Extract', ...
                     'units', 'normalized', 'position', [0.85 0.01 0.13 0.03]);

h.doublearea_tag = uicontrol('Style', 'radiobutton', 'string', 'Difference between ranges', ...
                     'units', 'normalized', 'position', [0.40 0.01 0.17 0.03]);
...
set(h.group_fname_button, 'callback', {@group_fnameCallback, h, fnames, datamatrix, xdata, chanlocs});
set(h.group_condition_button, 'callback', {@group_conditionCallback, h, condition, datamatrix, xdata, chanlocs});
set(h.difference_button, 'callback', {@differenceCallback, h, datamatrix, xdata, chanlocs});
set(h.subjpopup, 'callback', {@editCallback, h, datamatrix, xdata, chanlocs});
set(h.ymaxedit, 'callback', {@editCallback, h, datamatrix, xdata, chanlocs});
set(h.yminedit, 'callback', {@editCallback, h, datamatrix, xdata, chanlocs});
set(h.extract_button, 'callback', {@extractCallback, h, datamatrix, xdata, fnames, eventcount, chanlocs, atype});
set(h.extract_raw_button, 'callback', {@extract_rawCallback, h, datamatrix, xdata, fnames, eventcount, chanlocs, atype});
set(h.topoplot_button, 'callback', {@topoplotCallback, h, datamatrix, xdata, fnames, eventcount, chanlocs});
...
if subj > length(datamatrix)
    ...
    for i=1:length(combined)
        hcurve = drawData(h, combined(i).data, xdata{1}, ymin, ymax, chanlocs, i);
        
        % create 'legends' to specify the colors
        hlegend(i) = uicontrol('Style', 'text', 'string', combined(i).id, ...
                     'units', 'normalized', 'position', [0.85 0.98-i*0.035 0.14 0.03], ...
                     'ForegroundColor', get(hcurve, 'color'), 'backgroundcolor', 'white', ...
                     'fontunits', 'normalized', 'fontsize', 0.8, 'horizontalalignment', 'right', ...
                     'ButtonDownFcn', {@legendCallback, i, h, datamatrix, xdata, chanlocs}, ...
                     'tooltipstring', 'Right-click to remove grouping.');
    end
...
function extract_rawCallback(~, ~, h, datamatrix, xdata, fnames, eventcount, chanlocs, atype)

outputtype = questdlg('Would you like to use an xls-file or separate csv-files for output?', 'Output-file type', ...
    'Xls', 'Csv', 'Xls');
...
switch outputtype
    ...
    case 'Csv'
        ...
        for j=1:length(combined)
            xlswrite([targetpath filesep combined(j).id '.csv'], combined(j).data);
            waitbar(i+j/length(datamatrix), hwait);
        end
...
function extractCallback(~, ~, h, datamatrix, xdata, fnames, eventcount, chanlocs, atype)
...
for k = subjvector
   ...
   % SAMPLE = calcMetric(stype, datamatrix{k}(:, ind_xmin:ind_xmax), range); 
    ...
    writeSampleRow(fid, SAMPLE, fnames{k}, stype, atype, num2str(eventcount{k}), ...
...
function topoplotCallback(~, ~, h, datamatrix, xdata, fnames, eventcount, chanlocs)
