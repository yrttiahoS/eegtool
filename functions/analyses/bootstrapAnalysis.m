function bootstrapAnalysis(dpath, filenames_to_analyze)
    ...
    %
    % Bootstrapping analyses are described in Yrttiaho, Forssman, 
    % Kaatiala, and Leppänen (2014)
    ...
    while repeat_questions
        ...
        [selection, ok] = listdlg('ListString', events, ...
                                  'SelectionMode', 'multi', ...
                                  'promptstring', ...
                                  'Select event(s) to use in analysis:');
        ...
        prompt = {'Number of bootstrap samples:', 'Number of epochs', ...
                  'EEG-Channel or Channel-group'};
        def = {'1000', num2str(size(cGrandwave, 3)), '1'};
        answer = inputdlg(prompt, 'Bootstrap analysis', 1, def);

        % if answer was empty first condition to be evaluated: did user
        %press quit? second condition to be evaluated: did user press
        % okay with empty?
        if isempty(answer)
            % repeat
        else
    ...
    cGrandwave = reshape(mean(cGrandwave(channel,:,:),1), 1, ...
                         size(cGrandwave,2), size(cGrandwave,3));
    ERP = ERPbootsrap(cGrandwave, bootsamples, numerps, 1, EEG.times, ...
                      color);
    xlabel 'time(ms)';
    ylabel 'U(uV)';
    waitbar(1, hwait);
    ...
    button = questdlg('Save bootstrap-analysis information to csv?', ...
