...
% Calculate FFT for each file and displays the results on a special GUI.
...
for i=1:length(filenames_to_analyze)
	...
	% xdata = frequency
	[datamatrix{i} xdata{i} eventcount{i}] = calculateFFT(EEG.data, EEG.srate, 1);
...
visualize1d(fnames, condition, datamatrix, xdata, eventcount, EEG.chanlocs, 'FFT', [0 10], {'Frequency (Hz)', '|Y(f)|'});