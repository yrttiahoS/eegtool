...
%  options       = cell with strings inside to provide options for the gui
...
%  option        = selected option from the list ([] means user did not quit by
%                  pressing 'Ok'.)
...
h.readyb = uicontrol('Style', 'pushbutton', 'string', 'Ok', ...
                   'horizontalalignment', 'center', 'position', [5 5 190 30]);
...
vector = str2num(get(h.vectoredit, 'string')); %#ok<*ST2NM>
