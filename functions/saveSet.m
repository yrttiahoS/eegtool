function saveSet(dpath, filename, EEG, ev_val, urev_val)
    ...
    %  ev_val   = ev_val matrix that indicates good/bad epochs
    %             [nchan nepochs]
    ...
    % save set-file
    pop_saveset(EEG, 'filename', [dpath filesep filename2], 'check', ...
                'on', 'savemode', 'onefile');
    ...
    fprintf(fid, [filename2 ',' num2str(size(urev_val, 2)-...
                  length(interpolated_per_column)) ',' ...
                  num2str(length(interpolated_per_column)) ',' ...
                  num2str(max(interpolated_per_column)) ',' ...
