function varargout = eegtoolPreprocess(d_path, d_out_path, filename, ...
                                           savemode, continuous_procedure, ...
                                           stim_ids, epoch_limits, ...
                                           bad_epochs, epoched_procedure)
    % ------------------------------------------------------------------------------
    ...
    h.epochlimtext = uicontrol('Style', 'text', 'string', '', 'units', 'normalized', ...
                               'position', [0.01 0.96 0.15 0.03], 'horizontalalignment', 'left');
...
function buttonPressControl(~,evnt)
    % Callback for button press on gui
...
function buttonReleaseControl(~,evnt)
    % Callback for button release on gui
...
function closereq_eegvideo(~, ~, heegtool, objh)
    % Closerequestfcn for the eegvideo figure
...
function imptresh_callback(~,~, h, hi, event_validity, impedancevector)
    % Callback for impedance treshold
...
function baselinec_callback(~, ~ ,h)
    % Callback for baseline-correction
...
function edit_callback(~, ~, h)
    % Callback for ymax-field interaction
...
function errorbutton_callback(~, ~, h)
    % Callback for Artefact detection-button
...
function ev_val_new = combine_ev(ev_val_new, ev_val, overwrite_old_ad)
    % Check ev_val_new does not display already interpolated epochs as
...
function rejepochs_callback(~,~, h)
    % Callback for Reject epochs-button
...
function disprejected_callback(~,~)
    % Callback for display rejected-button
...
function interpbutton_callback(~, ~, h)
    % Callback for interpolate-button
...
function rmepochbutton_callback(~, ~, h)
    % Callback for remove epoch-button
...
function filtbutton_callback(~, ~, h)
    ...
    % Filter continuous data and replace the continuos data in the memory with
    % filtered data
...
function rerefbutton_callback(~, ~, h)
    % Callback for rereferencing button
...
function clearDatastructs(h)
    % Removes the necessary datastructs
...
function clearFields(h)
    % Formats the informative fields to zero-state
...
function drawContData(starttime, winlen, h)
    % Draws all the axes objects in the head (continuous)
    % parameters: 
...
function contClick(~,~)
    % Callback for mouseclick from continuous data

    % what kind of click was performed?
    clicktype = get(gcf, 'SelectionType');

    % if normal click -> zoom
...
function drawEpochData(nstim, nepoch, h)
    % Draws all the axes objects in the head (epoched)
    % parameters:
...
function axesclick(~,~, channel, nepoch, nevent)
    % Callback for mouse click; different actions for different type of clicks
    % (normal, open, alt, extended)
...
function moveSelection(position, h)
    % Callback to move to a certain point in the data. 
...
function switchContinuous(h)
    % Callback when switching to continuous mode.
...
function controlSlider(EEG, sliderlocation, h)
    % Sets slider first_event_number to last_event_number
...
function eventids = generateEventIds(ALLEEG)
    % Returns event id's according to the setnames on the ALLEEG-struct EEG's
